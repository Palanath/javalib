...
public abstract class Background {
	 ...
	 * </p>
	 */
	private final BooleanProperty runOnApplicationThread = new SimpleBooleanProperty(true);
	...
	private boolean run;
	{
		final ChangeListener<Number> widthListener = (observable, oldValue, newValue) -> resized(newValue.doubleValue(),
	...
	{
		final int refreshRate = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
	...
	public Background(final Canvas canvas) {
		setCanvas(canvas);
	}

	protected Background(final Canvas canvas, boolean autoclear) {
		this(canvas);
		autoclear = false;
	}

	public Background(final Canvas canvas, final double targetFramerate) {
		this(canvas);
		setTargetFramerate(targetFramerate);
	}

	protected Background(final Canvas canvas, final double targetFramerate, boolean autoclear) {
		this(canvas, targetFramerate);
		autoclear = false;
	}

	public Background(final double targetFramerate) {
		setTargetFramerate(targetFramerate);
	}
