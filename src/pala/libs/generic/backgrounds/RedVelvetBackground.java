package pala.libs.generic.backgrounds;
...
public class RedVelvetBackground extends Background {
	...
	private class Circle {
		...
		public void draw() {
			...
			final GraphicsContext gc = getContext();
			gc.setStroke(new Color(1, 1, 1, Math.abs(Math.abs(opacity) - 1)));
			gc.setLineWidth(1.5);
			gc.setEffect(DEFAULT_CIRCLE_EFFECT);

			final double diameter = sizeScale * 100 * (getWidth() + getHeight()) / 3000;// == 100 * sizeScale on a
																						// 1920x1080
	...
	protected RedVelvetBackground(final boolean autoclear) {
		super(autoclear);
	}

	public RedVelvetBackground(final Canvas canvas) {
		super(canvas);
	}

	protected RedVelvetBackground(final Canvas canvas, final boolean autoclear) {
		super(canvas, autoclear);
	}

	public RedVelvetBackground(final Canvas canvas, final double targetFramerate) {
		super(canvas, targetFramerate);
	}

	protected RedVelvetBackground(final Canvas canvas, final double targetFramerate, final boolean autoclear) {
		super(canvas, targetFramerate, autoclear);
	}

	public RedVelvetBackground(final double targetFramerate) {
	...
	@Override
	protected void resized(final double width, final double height, final double oldWidth, final double oldHeight) {
		for (final Circle c : circles) {
			c.x *= width / oldWidth;
	 ...
	 */
	public void setCircleFadeTime(final float seconds) {
		steps.unbind();
	 ...
	 */
	public void setCircles(final int count) {
		circles.clear();
